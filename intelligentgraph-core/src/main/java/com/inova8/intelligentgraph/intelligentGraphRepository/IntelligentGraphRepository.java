...
import com.inova8.intelligentgraph.model.Graph;
...
//	}
	...
	public CustomQueryOptions getCustomQueryOptions(Value[] customQueryOptionsArray) {
		...
		if (customQueryOptions != null) {
			if (customQueryOptions.contains("service")) {
				com.inova8.intelligentgraph.model.IResource service = customQueryOptions.get("service");
	...
	private void clearServiceCache( CustomQueryOptions customQueryOptions) {
		if (connected()) {
	...
	private String getBefore(CustomQueryOptions  customQueryOptions) {
		if (customQueryOptions != null && customQueryOptions.containsKey("before")) {
			com.inova8.intelligentgraph.model.IResource beforeDateTime = customQueryOptions.get("before");
...
//
	...
	public Thing getThing(String iri) throws RecognitionException, PathPatternException {
		IRI thingIri = PathParser.parseIriRef(this.getPrefixes(),iri).getIri();
		return getThing(thingIri);
	}

	public Thing getThing(IRI iri, ICustomQueryOptions customQueryOptions) {
		return Thing.create(this, iri, new EvaluationContext(customQueryOptions));
	}

	public Thing getThing(String iri, ICustomQueryOptions customQueryOptions) throws RecognitionException, PathPatternException {
		IRI thingIri = PathParser.parseIriRef(this.getPrefixes(),iri).getIri();
	...
	public Graph addGraph(String graphName) {
		...
		try {
			graphNameIri = PathParser.parseIriRef(this.getPrefixes(), graphName).getIri();
