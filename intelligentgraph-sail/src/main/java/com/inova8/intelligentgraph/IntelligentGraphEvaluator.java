...
public class IntelligentGraphEvaluator extends AbstractCloseableIteration<BindingSet, QueryEvaluationException> {
	...
	/** The query context. */
	private  ICustomQueryOptions  customQueryOptions = new ICustomQueryOptions();	
				...
				if (nextBindingSet.getValue("object").isLiteral()) {
					...
					if (Evaluator.getEngineNames().containsKey(literalValue.getDatatype())) {
						...
						Thing subjectThing = Thing.create(source, nextBindingSet.getValue("subject"),
								evaluationContext);
						try {
							com.inova8.intelligentgraph.IResource.Resource fact = subjectThing.getFact(
						...
						if (!statementPatterns.isEmpty()) {
							...
							if (boundStatement != null) {
								...
								case VALUE:
									...
									try {
										com.inova8.intelligentgraph.IResource.Resource fact = subjectThing
	 ...
	 */
	public ICustomQueryOptions getCustomQueryOptions(BindingSet nextBindingSet) {
		if (nextBindingSet.size() == 0) {
			return null;
		} else {
			ICustomQueryOptions customQueryOptions = new ICustomQueryOptions();
			for (Binding bindingValue : nextBindingSet) {
				...
				customQueryOptions.put(customQueryOptionParameter,
						IResource.create(getSource(), customQueryOptionValue, null));
			}
