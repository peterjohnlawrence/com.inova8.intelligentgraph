...
import com.inova8.intelligentgraph.model.ICustomQueryOptions;
import com.inova8.intelligentgraph.model.IntelligentRepositoryConnection;
 ...
 */
public class IntelligentGraphConnection extends NotifyingSailConnectionWrapper implements IntelligentRepositoryConnection  {

	public static ICustomQueryOptions customQueryOptions;
	...
	private CloseableIteration<? extends IntelligentStatement, SailException> getThingFacts(IntelligentGraphRepository source,Thing thing, PathElement pathElement,Resource... contexts ) throws PathPatternException {
		return (CloseableIteration<? extends IntelligentStatement, SailException>) 
				new IntelligentStatementFacts(source,thing, pathElement,this,customQueryOptions,contexts);
	}
	private CloseableIteration<? extends IntelligentStatement, SailException> traceThingFacts(IntelligentGraphRepository source,Thing thing, PathElement pathElement,Resource... contexts ) throws PathPatternException {
		return (CloseableIteration<? extends IntelligentStatement, SailException>) 
				new IntelligentStatementFacts(source,thing, pathElement,this,customQueryOptions,true,contexts);
	}	
...
//		pathElement.setCustomQueryOptions(customQueryOptions);
		PathTupleExpr pathTupleExpr = pathElement.pathPatternQuery(thing,pathIteration,customQueryOptions);
		TupleExpr tupleExpr = pathTupleExpr.getTupleExpr();
		SimpleDataset dataset = prepareDataset(pathElement, contexts);
		BindingSet bindings = new QueryBindingSet();
		EvaluationStrategy evaluationStrategy = new StrictEvaluationStrategy(source.getTripleSource(),dataset, null);
	...
	private SimpleDataset prepareDataset(PathElement pathElement,Resource... contexts)
		...
		ICustomQueryOptions customQueryOptions = prepareCustomQueryOptions(pathElement, contexts);
	...
	private ICustomQueryOptions prepareCustomQueryOptions(PathElement pathElement, Resource... contexts) {
		ICustomQueryOptions customQueryOptions = URNCustomQueryOptionsDecode.getCustomQueryOptions(contexts,getPrefixes());
		ICustomQueryOptions pathQLCustomQueryOptions = pathElement.getCustomQueryOptions();
