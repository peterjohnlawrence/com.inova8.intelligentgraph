...
//	private Prefixes prefixes;
	...
	public PathPatternVisitor(Prefixes prefixes) {
		super();
		this.repositoryContext = new RepositoryContext(prefixes, null);
	}
	...
	public QueryOptionsPathElement visitQueryOptions(QueryOptionsContext ctx) {
		...
		QueryOptionsPathElement queryOptionsPathElement = new QueryOptionsPathElement(repositoryContext);
...
//	}
	...
	public PathElement visitBoundPattern(PathPatternParser.BoundPatternContext ctx) { 
		...
		PathElement pathElement = new BoundPathElement(repositoryContext);
	...
	public CardinalityElement visitCardinality(CardinalityContext ctx) {
		...
		CardinalityElement cardinalityElement = new CardinalityElement(repositoryContext);
	...
	public SequencePathElement visitPathSequence(PathSequenceContext ctx) {
		...
		if (ctx.pathPatterns().size() == 1) {
			...
			SequencePathElement pathSequenceElement = new SequencePathElement(repositoryContext);
	...
	public AlternativePathElement visitPathAlternative(PathAlternativeContext ctx) {
		...
		if (ctx.pathPatterns().size() == 1) {
			...
			AlternativePathElement pathAlternativeElement = new AlternativePathElement(repositoryContext);
	...
	public PredicateElement visitPredicate(PredicateContext ctx) {
		...
		if (ctx.reifiedPredicate() != null) {
			...
			predicateElement = new PredicateElement(repositoryContext);
			predicateElement.setOperator(PathConstants.Operator.PREDICATE);
	...
	public PredicateElement visitReifiedPredicate(ReifiedPredicateContext ctx) {
		...
		PredicateElement reifiedPredicateElement = new PredicateElement(repositoryContext);
	...
	public IriRefValueElement visitPredicateRef(PredicateRefContext ctx) {
		...
		if (ctx.qname() != null) {
			...
			predicateRefElement = new IriRefValueElement(repositoryContext);
	...
	public IriRefValueElement visitIriRef(IriRefContext ctx) {
		...
		IriRefValueElement iriRefElement = new IriRefValueElement(repositoryContext);
		if (ctx.qname() != null) {
	...
	public FactFilterElement visitPropertyListNotEmpty(PropertyListNotEmptyContext ctx) {
		...
		FactFilterElement propertyListNotEmptyElement = new FactFilterElement(repositoryContext);
		...
		for (VerbObjectListContext verbObjectListContext : ctx.verbObjectList()) {
			...
			VerbObjectList verbObjectList = new VerbObjectList(repositoryContext);
	...
	public ObjectListValueElement visitObjectList(ObjectListContext ctx) {
		...
		for( ObjectContext objectContext: ctx.object()) {
			if(objectContext.iriRef()!=null) {
				...
				BindVariableElement bindVariableElement = new BindVariableElement(repositoryContext);
		...
		ObjectListValueElement objectListElement = new ObjectListValueElement(repositoryContext);
	...
	public IriRefValueElement visitQname(QnameContext ctx) {
		...
		IriRefValueElement qnameElement = new IriRefValueElement(repositoryContext);
		IRI qname = Utilities.convertQName(ctx.getText(),getPrefixes());
		if (qname!=null) {
	...
	public IriRefValueElement visitPname_ns(Pname_nsContext ctx) {
		...
		IriRefValueElement pname_nsElement = new IriRefValueElement(repositoryContext);
		Prefixes prefixes=null;
		//prefixes=source.getPrefixes();
	//	if(thing!=null) prefixes=thing.getPrefixes();
		IRI qname = Utilities.convertQName(ctx.getText(),prefixes);
		if (qname!=null) {
	...
	public LiteralValueElement visitLiteral(LiteralContext ctx) {
		...
		LiteralValueElement literalElement = new LiteralValueElement(repositoryContext);
	...
	public FilterOperatorValueElement visitOperator(OperatorContext ctx) {
		...
		FilterOperatorValueElement operatorElement = new FilterOperatorValueElement(repositoryContext);
