		...
		if (getCardinality(pathIteration) > 0) {
			...
			for (int iteration = 1; iteration <= getCardinality(pathIteration); iteration++) {
				...
				if (leftPattern==null){
					...
					rightPattern = getRightPathElement().pathPatternQuery(
							/*thing,*/ intermediateVariable, predicateVariable, intermediateTargetVariable,
							pathIteration,customQueryOptions);
				}else {
					rightPattern = getRightPathElement().pathPatternQuery(/*thing,*/ leftPattern.getStatementBinding().getTargetVariable() /*intermediateVariable*/,  predicateVariable, intermediateTargetVariable,
							pathIteration,customQueryOptions);
				}
